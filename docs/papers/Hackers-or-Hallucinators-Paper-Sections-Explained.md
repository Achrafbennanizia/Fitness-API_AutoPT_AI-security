...
This file and the two mind-map files are three views of the same paper, not three papers. [Hackers-or-Hallucinators-Mindmaps.md](Hackers-or-Hallucinators-Mindmaps.md) holds flowchart diagrams because Cursor’s preview does not render Mermaid `mindmap` syntax. [Hackers-or-Hallucinators-Mindmaps-Explained.md](Hackers-or-Hallucinators-Mindmaps-Explained.md) is a short spoken tour of each diagram. **This file** is the long companion that follows the PDF’s own headings at 300–500 words each. When a diagram feels thin, come here. When this prose feels long, open the matching map for the tree of claims.
